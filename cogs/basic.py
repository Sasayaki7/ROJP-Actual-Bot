...
class Basic(commands.Cog):
    ...
    __slots__ = ["bot", "akemiversion", "__changelog", "versions", "__changelog_en", "data_getter", "languages", "prefixes"]


    def __init__(self, bot):
        self.bot = bot
        self.akemiversion = "1.2.0"
        self.__changelog=["[JP]:-役職権限追加[EN]:-added ability to give roles",
            "[JP]:-リロード機能追記\n-analyticsコマンド追記[EN]:-added analytics command\n-added reload commands",
            "[JP]:-link無しでもlinkコマンド使える\n-バグ修正\n-タグ重複検知\n-サーバーデータコマンド追加\n[EN]:-link can be used without explicitly typing out link\n-Fixed bugs\n-bot can detect duplicate tags in link\n-Added server stat commands",
            "[JP]:-linkのファジー検索追加\n-変更ログの見やすさアップ[EN]:-Added fuzzy identification for link command\n-Improved changelog visuals",
            "[JP]:-セキュリティ更新\n-0票リーダーボードバグ修正[EN]:-Security update\n-Fixed 0 count leaderboard bug",
            "[JP]:-プログラムを根本的に書き換えた\n-データベース導入\n-Cogs導入\n-リーダーボードの見栄えアップ[EN]:-Rewrote bot\n-Implemented Database\n-Implemented Cogs\n-Improved Leaderboard aesthetics",
            "[JP]:-uptimeの表示バグを修正\n-linklistコマンドの追加\n-スターボードの画像だけ投稿の画像を表示する。\n-エラーメッセージ一部追加\n-挨拶機能削除[EN]:-Fixed uptime display glitch\n-Added linklist command\n-Made images visible on starboard posts\n-Added some error messages\n-Removed 'hello' feature",
            "[JP]:-uptime、version、about、changelog、linkコマンドを追加\n-挨拶機能、メンション機能を追加。\n-擬人化した言葉遣い\n-データ破損、損失防止システム\n-画像だけのメッセージもスターボードに投稿できるようになった。\n-バックアップ機能追加[EN]:-Added commands uptime, version, about, changelog, link\n-Added greeting and mention features\n-Personified アケミ\n-Backup system added\n-Can add empty messages to starboard\n-Added backup feature part 1"]
        self.versions = [f"[JP]:**変更ログ v{x}**[EN]:**Change Log v{x}**" for x in ["1.2.0","1.1.0","1.0.3","1.0.2", "1.0.1", "1.0.0", "0.2.1", "0.2.0"]]
    ...
    @commands.command(help="[JP]:私のverを表示します。[EN]:Shows the current version of アケミ", aliases=["ver"])
    async def version(self, ctx):
        author = self.bot.user
        embed=discord.Embed(color=0x0000ef, description=self.bot.content_to_lang(f"[JP]:私はversion{self.akemiversion}です。[EN]:Current version: {self.akemiversion}", ctx))
        embed.set_author(name=author.name, icon_url=author.avatar_url)
        embed.timestamp = datetime.datetime.now()
        await ctx.send(embed=embed)
    ...
    async def about(self, ctx):
        ...
        embed=discord.Embed(color=0x0000ef, title=self.bot.content_to_lang("[JP]:自己紹介[EN]:About myself",ctx),
            description=self.bot.content_to_lang(f"[JP]:私はサーバー内での会話をより楽しめるために\
        作り上げられたボットです。\n挨拶、新メンバーの歓迎、スターボード等の機能がついています。\nコマンド表は　``{ctx.prefix}help``で閲覧できます。\n提案、バグなど\
        があれば<@!346438466235662338>へDMお願いします。24時間以内に返答します。[EN]:I was created to help socialization in servers be \
        more enjoyable.\nI welcome new members, have starboard features, etc. \nA list of commands can be seen by \
        typing ``{ctx.prefix}help``.\nAny suggestions and feedback should go to <@!346438466235662338>. I \
        will respond within 24 hours", ctx))

        embed.set_author(name=author.name, icon_url=author.avatar_url)
        invite_link="https://discordapp.com/api/oauth2/authorize?client_id=638188470757752842&permissions=93248&scope=bot"
        embed.add_field(name=self.bot.content_to_lang("[JP]:サーバーへの招待[EN]:Invite", ctx),
            value=self.bot.content_to_lang(f"[JP]:アケミをあなたのサーバーに誘いたい場合は[こちら]({invite_link})をクリックください！\
    ...
    async def on_member_join(self, member):   #When a member joins, we send them a nice welcome message that is set by the guild using the welcome command to the specified welcome_channel
        await member.send(f"{await self.getParameters(member.guild.id, 'welcome_message')}")
        await self.data_getter.increment_entry(member.guild.id)
    ...
    @commands.guild_only()
    @commands.check(has_permissions)
    @language.command(help="[JP]:language chan: Changes language for this channel only/このチャンネルのみアケミちゃんの言語を変えます。[EN]:language chan: Changes language for this channel only/このチャンネルのみアケミちゃんの言語を変えます。")
